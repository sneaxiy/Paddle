...
static VarDesc *TryGetLatestVarDesc(
    const std::pair<std::string, std::vector<VarHandle *>> &name_var_pair) {
  VarDesc *var_desc = nullptr;
  find_if(name_var_pair.second.rbegin(), name_var_pair.second.rend(),
...
std::unique_ptr<ir::Graph> ReferenceCountPass::ApplyImpl(
  ...
  PADDLE_ENFORCE(last_live_ops_of_vars.empty() && ref_cnts.empty(),
                 "Last Live Ops of vars should be initialized at here.");
  ...
  for (size_t i = 0; i < vars.size(); ++i) {
    for (auto &name_var_pair : vars[i]) {
      ...
      VarDesc *var_desc = TryGetLatestVarDesc(name_var_pair);
      if (var_desc == nullptr || var_desc->Persistable()) {
        continue;
      }
      if (var_desc->GetType() != proto::VarType::LOD_TENSOR &&
          var_desc->GetType() != proto::VarType::SELECTED_ROWS &&
          var_desc->GetType() != proto::VarType::LOD_TENSOR_ARRAY) {
        // Var type cannot be delete.
        continue;
      }
